...
contracts::Result Dispatch(const contracts::commands::Command& c) override {std::scoped_lock l(m_); auto it=handlers_.find(c.id); if(it==handlers_.end()) return contracts::Result::Failure(contracts::ErrorCode::NotImplemented); return it->second(c);}
