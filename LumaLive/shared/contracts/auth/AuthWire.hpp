...
inline std::string encode(const Packet& p) {
    std::string o=std::to_string((unsigned)p.type);
    o.push_back('|');
...
inline Packet decode_line(std::string_view s) {
    std::vector<std::string> v;
    std::string c;
    for(char x:s){
        if(x=='\r'||x=='\n')continue;
        if(x=='|'){v.push_back(std::move(c));c.clear();}
        else c.push_back(x);
    }
    v.push_back(std::move(c));
    if(v.empty()||v[0].empty())throw std::invalid_argument("invalid packet");
    auto n=std::stoul(v[0]);
    if(n>255)throw std::invalid_argument("invalid packet type");
    Packet p{static_cast<Type>(n),{}};
    v.erase(v.begin());
    p.fields=std::move(v);
    return p;
}

inline bool valid_field(std::string_view s){
