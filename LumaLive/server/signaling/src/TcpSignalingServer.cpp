...
bool TcpSignalingServer::Start(std::uint16_t port){if(running_||!sockets())return false;luma_socket_t s=::socket(AF_INET,SOCK_STREAM,0);if(!valid_socket(s))return false;int yes=1;setsockopt(s,SOL_SOCKET,SO_REUSEADDR,reinterpret_cast<const char*>(&yes),sizeof(yes));sockaddr_in a{};a.sin_family=AF_INET;a.sin_addr.s_addr=htonl(INADDR_ANY);a.sin_port=htons(port);if(bind(s,reinterpret_cast<sockaddr*>(&a),sizeof(a))!=0||listen(s,32)!=0){close_socket(s);return false;}listen_socket_=s;running_=true;accept_thread_=std::thread(&TcpSignalingServer::AcceptLoop,this);return true;}
void TcpSignalingServer::Stop(){running_=false;if(valid_socket(listen_socket_)){shutdown(listen_socket_,2);close_socket(listen_socket_);listen_socket_=kLumaInvalidSocket;}if(accept_thread_.joinable())accept_thread_.join();{std::lock_guard lock(mutex_);for(const auto&[s,c]:clients_){(void)c;shutdown(s,2);}}for(auto& thread:client_threads_)if(thread.joinable())thread.join();client_threads_.clear();{std::lock_guard lock(mutex_);clients_.clear();rooms_.clear();}}
...
#endif
luma_socket_t s=accept(listen_socket_,reinterpret_cast<sockaddr*>(&a),&n);if(!valid_socket(s)){if(running_)continue;break;}std::lock_guard lock(mutex_);clients_.emplace(s,Client{s,{},{}});client_threads_.emplace_back(&TcpSignalingServer::ClientLoop,this,s);}}
