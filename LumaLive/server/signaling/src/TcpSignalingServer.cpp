...
void TcpSignalingServer::Stop(){std::lock_guard lifecycle_lock(lifecycle_mutex_);if(!running_.load()&&!accept_thread_.joinable())return;running_=false;if(accept_thread_.joinable())accept_thread_.join();{std::lock_guard lock(listen_mutex_);if(valid_socket(listen_socket_)){shutdown(listen_socket_,2);close_socket(listen_socket_);listen_socket_=kLumaInvalidSocket;}}{std::lock_guard lock(mutex_);for(const auto&[s,c]:clients_){(void)c;shutdown(s,2);}}for(auto& thread:client_threads_)if(thread.joinable())thread.join();client_threads_.clear();{std::lock_guard lock(mutex_);clients_.clear();rooms_.clear();}}
...
#endif
if(setsockopt(s,SOL_SOCKET,SO_SNDTIMEO,reinterpret_cast<const char*>(&send_timeout),sizeof(send_timeout))!=0){close_socket(s);continue;}
...
void TcpSignalingServer::ClientLoop(luma_socket_t s){Client connection;{std::lock_guard lock(mutex_);auto it=clients_.find(s);if(it==clients_.end())return;connection=it->second;}while(running_){std::uint32_t n=0;if(!recv_all(s,reinterpret_cast<std::uint8_t*>(&n),4))break;n=ntohl(n);if(n==0||n>256*1024)break;std::vector<std::uint8_t>b(n);if(!recv_all(s,b.data(),n))break;luma::contracts::SignalingMessage m;if(!luma::contracts::wire::decode(b,m))break;Client c;{std::lock_guard lock(mutex_);auto it=clients_.find(s);if(it==clients_.end())break;c=it->second;}HandleMessage(c,m);{std::lock_guard lock(mutex_);if(clients_.find(s)==clients_.end())break;}}RemoveClient(s);{std::lock_guard lock(connection.send->mutex);connection.send->open=false;close_socket(s);}}
