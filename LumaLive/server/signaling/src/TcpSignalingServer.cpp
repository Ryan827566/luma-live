...
bool TcpSignalingServer::Send(int socket,const luma::contracts::SignalingMessage&m){auto b=luma::contracts::wire::encode(m);if(b.size()>16*1024*1024)return false;std::uint32_t n=htonl(static_cast<std::uint32_t>(b.size()));return send_all(socket,reinterpret_cast<std::uint8_t*>(&n),4)&&send_all(socket,b.data(),b.size());}
